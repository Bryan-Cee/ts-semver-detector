      ...
      for (const [name, oldDeclarations] of oldExportMap) {
        if (!newExportMap.has(name)) {
          try {
            ...
            console.error(
              `Error processing removed declaration ${name}:`,
              error
            );
          }
        }
        ...
        try {
          ...
          for (const rule of this.rules) {
            try {
              ...
              console.error(
                `Error in rule ${rule.constructor.name} for ${name}:`,
                ruleError
              );
            }
          }
          ...
          console.error(
            `Error processing declaration ${name}:`,
            declarationError
          );
        }
      }
    ...
    try {
      ...
      if (ts.isVariableDeclaration(node)) {
        if (node.name && ts.isIdentifier(node.name)) {
          ...
          node.name &&
          typeof (node.name as ts.Node).getText === "function"
        ) {
      ...
      console.error(
        `Error getting declaration name for node kind: ${
